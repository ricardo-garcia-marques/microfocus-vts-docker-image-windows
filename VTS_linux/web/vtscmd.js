...
function deleteAll(port, params) {
    params.port = port;
    common.sendRequest(configure.adminPort, 'delete_instance', params, function (e, trunk) {
        if (e) {
            logger.error(e);
            return;
        }
                ...
                if (!obj.success) {
                    logger.error('Delete failed:', obj.msg);
                }
                else {
...
function importFile(port, params, filePath, delimiter) {
    ...
    Util.copyFile(source, target, function (err) {
        if (!Util.empty(err)) {
            ...
            common.sendRequest(configure.adminPort, 'import_local_file', params, function (e, trunk) {
                if (e) {
                    logger.error(e);
                    return;
                }
